document.getElementById('convertButton').addEventListener('click', async function () {
 ...
 const apiUrl = "https://api.coingecko.com/api/v3/coins/avalanche-2/market_chart?vs_currency=usd&days=7";

        async function fetchAvalancheChartData() {
            try {
                const response = await fetch(apiUrl);
                const data = await response.json();
                ...
                // Render the chart
                renderAvalancheChart(labels, prices);
            } catch (error) {
                console.error("Error fetching Avalanche chart data:", error);
            }
        }

        function renderAvalancheChart(labels, prices) {
            const ctx = document.getElementById("avaxChart").getContext("2d");
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: prices,
                        borderColor: "#e84142",
                        backgroundColor: "rgba(0, 216, 214, 0.2)",
                        borderWidth: 2,
                        tension: 0.2,
                        pointRadius: 3
                    }]
        ...
        // Fetch and render Avalanche chart data
        fetchAvalancheChartData()