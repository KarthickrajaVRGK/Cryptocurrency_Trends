document.getElementById('convertButton').addEventListener('click', async function () {
   ...
   // TRON API URL
    const apiUrl = "https://api.coingecko.com/api/v3/coins/tron/market_chart?vs_currency=usd&days=7";

    async function fetchTronChartData() {
        try {
            const response = await fetch(apiUrl);
            const data = await response.json();
            ...
            // Render the chart
            renderTronChart(labels, prices);
        } catch (error) {
            console.error("Error fetching TRON chart data:", error);
        }
    }

    function renderTronChart(labels, prices) {
    ...
    // Fetch and render TRON chart data
    fetchTronChartData();