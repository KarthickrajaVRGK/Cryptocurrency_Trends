...
const apiUrl = "https://api.coingecko.com/api/v3/coins/dogecoin/market_chart?vs_currency=usd&days=7";
    
            async function fetchDogecoinChartData() {
                try {
                    const response = await fetch(apiUrl);
                    const data = await response.json();
                    ...
                    // Render the chart
                    renderDogecoinChart(labels, prices);
                } catch (error) {
                    console.error("Error fetching Dogecoin chart data:", error);
                }
            }
    
            function renderDogecoinChart(labels, prices) {
            ...
            // Fetch and render Dogecoin chart data
            fetchDogecoinChartData();